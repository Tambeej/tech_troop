...
let employees = { ax01: 1300, qs84: 840, bg33: 2700 };

const findEmployeeSalary = (employeeID) => employees[employeeID];
...
const findIndex = function (numbers, num) {
  console.log(findIndexRecursive(numbers, num, 0));
};

const findIndexRecursive = function (numbers, num, index) {
  let mid = Math.ceil(numbers.length / 2);
  let middleValue = numbers[mid];
  if (numbers.length == 1) {
    return index;
  }
  if (middleValue == num) {
  ...
  if (middleValue < num) {
    return findIndexRecursive(numbers.slice(mid), num, index + mid);
  }
  if (middleValue > num) {
    return findIndexRecursive(numbers.slice(0, mid), num, index);
  }
};

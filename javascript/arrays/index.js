...
let usersMailCompany = users.map((user) => {
  return { email: user.email, companyName: user.company.name };
});

console.log(usersMailCompany);

//Ex 2.

const startWithFive = (a) => a.address.zipcode[0] == "5";
let zipCodeStratsWithFive = users.filter(startWithFive);

console.log(zipCodeStratsWithFive);

//Ex 3.

let idZipCodeStratsWithFive = zipCodeStratsWithFive.map((user) => user.id);
console.log(idZipCodeStratsWithFive);

//Ex 4.

const startWithC = (a) => a[0].toLowerCase() == "c";
let userNameC = users.map((user) => user.name).filter(startWithC);
...
const findBySuite = (suite) =>
  users.find((user) => user.address.suite === suite);
const companyName = findBySuite("Apt. 950").company.name;
...
let scoresCounts = studentScores.reduce(
  (scores, score) => {
    if (score >= 90) {
      scores["A"]++;
    } else if (score >= 80) {
...
let totalCost = cartItems.reduce(
  (sum, item) => sum + item.price * taxRates[item.category] + item.price,
  0
);
